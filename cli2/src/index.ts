...
const programId = new PublicKey('preCY7QmspyxB2anVKjvxgvxVbKg4rwQVK1ffPhTk8Y')
const pool_address = new PublicKey('9CWA9Ca7xCB8cqoXTMM3eR83xNrW3Maqu2i7WcA6VYjs')
const withdrawer = new PublicKey('E4xZQpNFmLhzrck35nBZAdUHYPMhGGWUegiY7XxWccrd');
const idl=JSON.parse(fs.readFileSync('../contract2/target/idl/presale.json','utf8'))
...
programCommand('get_whitelist_info')
.requiredOption(
  '-k, --keypair <path>',
  'Solana wallet location'
)
.action(async (directory,cmd)=>{
  ...
  const provider = new anchor.Provider(conn,wallet,confirmOption)
  const program = new anchor.Program(idl,programId,provider)
  const [data, bump] = await getWhitelistInfoAccount(owner.publicKey, pool_address);
  const whitelistInfoData = await getWhitelistInfo(conn, data);
  // console.log(contributeInfoData);
  console.log("============ WhitelistInfo Data ===========");
  console.log("Pool : " + whitelistInfoData.pool.toBase58())
  console.log("Contributer : " + whitelistInfoData.contributer.toBase58())
  console.log("Whitelist : " + whitelistInfoData.whitelist)
  console.log("Contribute Start : " + whitelistInfoData.contributeStart.toNumber())
  console.log("Contribute Last : " + whitelistInfoData.contributeLast.toNumber())
  console.log("Amount : " + whitelistInfoData.amount.toNumber())
  console.log("")
})

programCommand('init_whitelist_info')
.requiredOption(
  '-k, --keypair <path>',
  'Solana wallet location'
)
.action(async (directory,cmd)=>{
  ...
  const [data, bump] = await getWhitelistInfoAccount(owner.publicKey, pool_address);
  let transaction = new Transaction()
  transaction.add(program.instruction.initWhitelistInfo(
    new anchor.BN(bump),
    {
      accounts:{
        owner : owner.publicKey,
...
.action(async (directory,cmd)=>{
  ...
  let transaction = new Transaction()
  transaction.add(program.instruction.addWhitelist(
    true,
    {
      accounts:{
        owner : owner.publicKey,
...
.action(async (directory,cmd)=>{
  ...
  let transaction = new Transaction()
  transaction.add(program.instruction.removeWhitelist(
    false,
    {
      accounts:{
        owner : owner.publicKey,
